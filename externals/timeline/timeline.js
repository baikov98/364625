		...
		if (dates.length != 0) {
			...
			if ($itimeline.find('date').length != 0) {	// #8368 data => date
				...
				while(true) {
					...
					settings.dates.push({
						id: id,
						label: modelNS.DateUtils.getDateValue(currentDate, settings.bigFormat)
					});
...
modelNS.Timeline  = modelNS.BaseModelView.extend({
	...
	render: function () {
		...
		function showPopup(params) {
			...
			$popup.css({opacity:0, left:0, top:0});
			...
			var timeout = setTimeout(function() {
				...
				function showAfterImagesLoaded () {
				...
				// calculating size after all images loaded
				if ($imgs.length) {
					var loaded = 0;
					// count loaded images from cache
					$imgs.each(function () {if (this.complete && this.naturalWidth) loaded++});

					if (loaded == $imgs.length) {
						showAfterImagesLoaded();
					} else {	// wait not loaded images, then count sizes
						$imgs.load(function() {
							if (++loaded == $imgs.length) showAfterImagesLoaded();
						});
					}
				} else {
					showAfterImagesLoaded();
		...
		function resizeEvents(params) {
			$eventsArea.find('.event-layout.category' + params.categoryID).each(function() {
				...
				if ($event.attr('end')) {
					...
					$event.find('.event-periodline').animate({width:endX});
					$event.find('.event-markline.end').animate({left:endX + 40});	// stasrt left compensassion
					$event.find('.event-pointer.end').animate({left:endX + 40})
				}
		...
		function scrollDates(index) {
			...
			$('.eventDates, .events-area .events').animate({
				left: left
			}, 40);
		}
