...
modelNS.CarouselScheme = modelNS.BaseModelView.extend({
	...
	render: function() {
		...
		this.redrawLines = function () {
			this.drawLines(getElementsCenters(this.$childs), this.carouselCanvas[0].getContext('2d'));
		};
		...
		this.openCentralPopup = function(e) {
			...
			if (this.position == this.POSITION_LEFT) {
				...
				this.hideContent(function () {self.openCentralPopup(e)});	// #9445 если положить в moveElementsBack то не успевает скрыть контент
				return;
			}
			...
			central.animate({ top: newTop }, this.duration(500));
			setTimeout(function() {
				central.hide();
				// central.css({'top': top});
				showPopup('central');
		...
		this.closeCentralPopup = function() {
			...
			central.show();
			central.animate({ top: top }, this.duration(500));
		}

		this.elementRotate = function(item, elementJSON) {
			if (this.isDemoPause()) return this.saveDemoMoment();
			if (!this.openingElement) return;

			if (!item) item = this.getElementById(elementJSON.id);

			var angle = parseFloat(item.data('angle')),
			...
			var callback = function() {
				...
				// mark item is current, its at left.
			...
			self.rotate(rotation, rotationAngle, item.attr('data-count'), callback);
		};
		...
		this.openContent = function (elementJSON)	{
			...
			// 	return this.hideContent(function () {
			// 		 self.openElement(null, elementJSON);
			// 	});
			// // }

			// if (this.openingElement != elementJSON) {
			// 	return this.openElement(null, elementJSON);
			// }

			// this.openedContent = elementJSON;

			// return self.moving = false;
			var $elem = this.getElementById(elementJSON.id);
			this.moveElementsLeft(elementJSON, function() {
				self.showContent($elem, elementJSON);
			});
		}

		// создает и показывает контент плашки
		this.showContent = function($element, elementJSON) {
			if (this.isDemoPause()) return this.saveDemoMoment();
			if (!this.openingElement) return;
			...
			if (this.$model.find('.content-block').length == 0) {
				$openedBlock = $('<div class="content-block"></div>')
					.appendTo(this.$model);
				...
				$openedBlock = this.$model.find('.content-block').show();
		...
		this.rotate = function(rotation, rotationAngle, count, callback) {
			...
			var elements = this.$childs;  // optimization
			...
			this.timers.rotate = requestAnimationFrame(function rotate(time) {
				...
				var centers = getElementsCenters(self.$childs);
		...
		this.moveElementsLeft = function(elementJSON, callback) {
			...
			// 	left:centralLeft,
			// 	top:centralTop,
			// 	width:centralWidth*scale,
			// 	height:centralHeight*scale
			// }, function () { self.stopDrawLines() });
			// $central.find('.content').animate({zoom:scale})

			// $elements.animate({left: 0}, {	queue: false })
		};
		...
		this.movingAnimation = function($elements, callback, test) {
			self.moving = true;
			// var canvas = self.carouselCanvas[0],
			// 	ctx = canvas.getContext('2d');
			// var i = 0;
			// var interval = setInterval(function() {
			// 	var centers = getElementsCenters(self.$model.find('.carousel-element:not(\'.central\')').not('.hidden'));
			...
			// 	    	complete = false;
			// 	    }
			// 		angle = angleDiff < 0 ? angle + self.DEGREE: angleDiff > 0 ? angle - self.DEGREE: angle;
			// 		if (angleDiff != 0) {
			// 			self.setElementPositionByAngle($(this), angle);
			// 		}
			// 	});
